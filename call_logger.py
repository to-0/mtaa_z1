...
class CallLogger:
    ...
    def invitel(self, origin, destination):
        ...
        if self.active_calls.get(origin) is None:
            self.log_file.write("INVITE from " + origin +" to "+ destination+ " at "+time.strftime("(%H:%M:%S)", time.localtime())+ "\n")
    ...
    def ack(self, origin, destination):
        ...
        if self.cancel is not True and self.active_calls.get(origin) is None and self.unack_calls.get(origin) is not None:
            self.log_file.write("Call acknowledged at "+time.strftime("%H:%M:%S", time.localtime())+ "\n")
    ...
    def cancel_log(self, origin):
        if self.unack_calls.get(origin) is not None:
            self.log_file.write("Call cancelled at " + time.strftime("%H:%M:%S", time.localtime()) + "\n")
            self.unack_calls.pop(origin)

    def bye(self, origin):
        self.log_file.write("Call finished at " + time.strftime("(%H:%M:%S)", time.localtime())+"\n")
        self.call_in_process = False
